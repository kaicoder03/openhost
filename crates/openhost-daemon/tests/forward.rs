...
#[tokio::test]
async fn parse_request_head_rejects_oversized_head() -> DaemonResult<()> {
    let (port, _state) = spawn_upstream(UpstreamResponder::default()).await;
    let (_tmp, app) = build_daemon(port).await;
    let session = establish_connection(&app).await;

    // Send a 64 KiB head — twice the 32KB cap.
    let mut big_head = b"GET / HTTP/1.1\r\n".to_vec();
    for i in 0..2000 {
        big_head.extend_from_slice(format!("X-Header-{i}: value\r\n").as_bytes());
    }
