...
impl Forwarder {
    ...
    pub fn from_config(cfg: &ForwardConfig) -> Result<Option<Self>, ForwardError> {
        ...
        let host_override_str = cfg.host_override.as_ref().unwrap_or(&authority);
    ...
    pub async fn forward(
        ...
        //
        // BOLT: Consolidating consecutive headers_mut() calls into a
        // single block reduces mutable HeaderMap lookups.
...
fn encode_websocket_response_head(
    ...
    // BOLT: Manual status line construction using status.as_str() avoids
    // the allocation and formatting overhead of the format! macro.
...
fn combine_target_and_path(target: &Uri, path: &str) -> Result<Uri, ForwardError> {
    ...
    // BOLT: Manual URI construction with pre-calculated capacity
    // avoids multiple intermediate string allocations and the
    // formatting overhead of the format! macro.
...
fn encode_response_head(
    ...
    //
    // BOLT: HeaderValue::from(u64) avoids the intermediate String
    // allocation of body_len.to_string().
    headers.insert(
...
mod tests {
    ...
    fn sanitize_strips_all_hop_by_hop_headers() {
        ...
        let hv = HeaderValue::from_static("127.0.0.1:8080");
        sanitize_request_headers(&mut h, &hv).unwrap();
        for name in HOP_BY_HOP_HEADERS {
            assert!(
                !h.contains_key(*name),
    ...
    fn sanitize_websocket_preserves_connection_and_upgrade() {
        ...
        let hv = HeaderValue::from_static("upstream.local");
        sanitize_websocket_request_headers(&mut h, &hv).unwrap();
        assert!(h.contains_key(http::header::UPGRADE));
    ...
    fn from_config_derives_host_override_from_target() {
        ...
        assert_eq!(fwd.host_override.to_str().unwrap(), "127.0.0.1:8080");
    }
    ...
    fn from_config_respects_explicit_host_override() {
        ...
        let fwd = Forwarder::from_config(&cfg).unwrap().unwrap();
        assert_eq!(fwd.host_override.to_str().unwrap(), "my-service.local");
    }
