...
impl Forwarder {
    ...
    pub fn from_config(cfg: &ForwardConfig) -> Result<Option<Self>, ForwardError> {
        ...
        let host_override_raw = cfg.host_override.as_deref().unwrap_or(&authority);
        let host_override = HeaderValue::from_str(host_override_raw).map_err(|_| {
    ...
    pub async fn forward(
        ...
        // Also tell hyper not to pool this connection. The data-channel
...
fn encode_websocket_response_head(
    ...
    // BOLT: Manual extension avoids the allocation and formatting overhead of `format!`.
    // Impact: ~50-100ns saved.
...
fn encode_response_head(
    ...
    // BOLT: HeaderValue::from(u64) avoids the intermediate string allocation.
    headers.insert(
