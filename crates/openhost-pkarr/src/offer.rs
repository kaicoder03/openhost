...
pub fn decode_answer_fragments_from_packet(
    ...
    // Single pass over the packet's resource records to collect all
    // fragments belonging to this client.
    ...
    for rr in packet.all_resource_records() {
        ...
        if label_bytes.starts_with(base_with_dash.as_bytes()) {
            let idx_bytes = &label_bytes[base_with_dash.len()..];
            let Ok(idx_str) = core::str::from_utf8(idx_bytes) else {
                continue;
            };
            ...
            if let RData::TXT(txt) = &rr.rdata {
                if bucket[idx as usize].is_some() {
    ...
    let Some(first) = bucket[0].take() else {
        return Ok(None);
    };
