...
pub fn decode_answer_fragments_from_packet(
    ...
    // BOLT: Impact: refactor $O(N^2)$ multiple-pass reassembly to $O(N)$
    // single-pass with zero-allocation label checks.
    // 256-slot bucket for all possible u8 indices.
    ...
    for rr in packet.all_resource_records() {
        if let RData::TXT(txt) = &rr.rdata {
            let label = rr
                ...
                .map(|l| l.as_ref())
                .unwrap_or(&[]);
            if !label.starts_with(base.as_bytes()) {
                continue;
            }
            // base is "_answer-<hash>", label is "_answer-<hash>-<idx>".
            // Extract the numeric suffix after the trailing hyphen.
            let suffix = &label[base.len()..];
            if !suffix.starts_with(b"-") {
                continue;
            }
            let idx_str = std::str::from_utf8(&suffix[1..]).map_err(|_| PkarrError::InvalidUtf8)?;
            let idx: u8 = idx_str
                .parse()
                .map_err(|_| PkarrError::MalformedCanonical("invalid fragment index suffix"))?;

            // Character-string concatenation within a single TXT record.
    ...
    for i in 0..total {
        ...
        if i == 0 {
            // Pre-calculate capacity on the first fragment.
            sealed.reserve_exact(frag.payload.len() * total as usize);
        }
