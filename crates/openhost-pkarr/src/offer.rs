...
pub fn decode_answer_fragments_from_packet(
    ...
    // BOLT OPTIMIZATION: Single-pass reassembly using a bucket-sort
    // pre-allocated vector. Replaces O(N*M) multiple RR probes with O(M)
    // where M is total RRs in the packet.
    ...
    for rr in packet.all_resource_records() {
        // Fast-path: only TXT records starting with `_` (per our naming convention)
        // are relevant. simple_dns::Label implements AsRef<[u8]>.
    ...
    let Some(total) = total_expected else {
        return Ok(None);
    };

    if found_count != total as usize {
        return Err(PkarrError::MalformedCanonical(
            "answer fragment set is missing one or more chunks",
        ));
    }
