...
pub fn decode_answer_fragments_from_packet(
    ...
    // Optimize DNS fragment reassembly using a single pass over packet.all_resource_records()
    // with an O(N) bucket-sort by the numeric label suffix.
    let mut text_buckets: [Option<String>; 256] = {
    ...
    for rr in packet.all_resource_records() {
        if let RData::TXT(txt) = &rr.rdata {
            ...
            if label_bytes.starts_with(prefix.as_bytes()) {
                ...
                let Ok(idx_str) = core::str::from_utf8(suffix) else {
                    continue;
                };
                ...
                if text_buckets[idx as usize].is_some() {
                ...
                text_buckets[idx as usize] = Some(out);
            }
        }
    }

    // Return Ok(None) if fragment 0 is absent to maintain backward compatibility.
    let Some(first_text) = text_buckets[0].as_ref() else {
        return Ok(None);
    };
    ...
    for i in 1..total {
        let text = text_buckets[i as usize]
            .as_ref()
    ...
    let mut capacity = 0;
    for i in 0..total {
        capacity += buckets[i as usize].as_ref().unwrap().payload.len();
    }

    let mut sealed = Vec::with_capacity(capacity);
    for i in 0..total {
        let frag = buckets[i as usize].take().unwrap();
