...
- **New `openhost-pkarr-wasm` crate** at `crates/openhost-pkarr-wasm/` — wasm-bindgen shim over `openhost-pkarr`'s resolver read path. Four `#[wasm_bindgen]` exports: `decode_host_record`, `verify_record`, `decode_offer`, `decode_answer_fragments`. All four delegate to a plain-Rust `core` tier so host-target tests exercise the real decode/verify logic without needing a wasm runtime. `cdylib + rlib` crate-type; `[target.'cfg(target_arch = "wasm32")'.dependencies] getrandom = { features = ["js"] }` unblocks the `rand_core` → `getrandom` chain on wasm32.
