...
### Added (PR #28.2, browser-extension WASM resolver)

- **New `openhost-pkarr-wasm` crate** at `crates/openhost-pkarr-wasm/` — wasm-bindgen shim over `openhost-pkarr`'s resolver read path. Four `#[wasm_bindgen]` exports: `parse_host_record` (structural decode, unverified), `decode_and_verify` (single-pass decode + Ed25519 + 2-hour freshness check — the recommended path for any JS caller that will trust the returned record), `decode_offer`, and `decode_answer_fragments`. All four delegate to a plain-Rust `core` tier so host-target tests exercise the real decode/verify logic without needing a wasm runtime. `cdylib + rlib` crate-type; `[target.'cfg(target_arch = "wasm32")'.dependencies] getrandom = { features = ["js"] }` unblocks the `rand_core` → `getrandom` chain on wasm32.
- **`openhost-pkarr` feature split** — new `full` (default) and `resolver-only` features. `full` enables `tokio`, `async-trait`, and `pkarr/full-client` (relay + DHT client); `resolver-only` leaves only the sync codec + offer/answer decoders and `pkarr/signed_packet + keys`, which compiles cleanly for `wasm32-unknown-unknown`. `publisher` + `resolver` modules are gated behind `full`; `Publish` error variant is gated on `full` since `resolver-only` cannot produce it. Existing consumers (daemon, client, ffi) use default features and are unchanged; the `pkarr` workspace dep now pins `default-features = false` so consumers must opt in explicitly.
- **`extension/scripts/build-wasm.sh`** — `wasm-pack build` driver. Emits `extension/wasm/pkg/openhost_pkarr.{js,_bg.wasm}` that a service worker loads via ES-module `import`. `wasm-pack` + `wasm32-unknown-unknown` are the only prerequisites; no Node dependency. Bundled `wasm-opt` disabled via `[package.metadata.wasm-pack.profile.release]` because wasm-pack v0.14's pinned optimizer is too old for current rustc's bulk-memory output — `.wasm` stays ≈230 KB without opt.
- **`extension/src/dev/resolver-probe.js`** — dev-only harness that fetches a packet from the first responsive default Pkarr relay (`relay.pkarr.org`, `pkarr.pubky.app`, `pkarr.pubky.org`) and logs the output of all four exports to the service-worker devtools console. Not imported from `src/background.js` by default; operators uncomment two lines to run it.
- **`extension/manifest.json`** — `background.type = "module"` (required for `--target web` ES imports from the service worker), `host_permissions` for the three default Pkarr relays, and a CSP (`script-src 'self' 'wasm-unsafe-eval'; object-src 'self'`) that permits WASM instantiation in MV3.
- **`crates/openhost-pkarr-wasm/tests/wasm_smoke.rs`** — six host-target smoke tests that call the `core` tier directly against `MemoryPkarrNetwork`-style synthesized packet bytes. Covers good decode, bad pubkey rejection, signature-verify true/false against source/mismatched pubkeys, absent-offer, absent-answer, and full fragment reassembly + seal-open round-trip.
