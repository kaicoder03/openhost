...
- **`Host` header matters.** HASS has `http.trusted_proxies` / `http.use_x_forwarded_for` settings that will reject requests arriving with an unexpected Host. This example pins `host_override = "127.0.0.1:8123"` so the forwarded request looks indistinguishable from a direct `curl` against localhost.
