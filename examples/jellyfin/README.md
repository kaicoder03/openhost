...
- **Direct-play streaming is partial-only.** Jellyfin's `/Videos/{id}/stream` endpoint honours `Range: bytes=…` and the openhost forwarder passes the header through unchanged, so small files play end to end. Larger files are limited by `max_body_bytes` (the daemon buffers each response fully — see the personal-site gotchas). Transcoded playback sessions open a WebSocket for session management + progress; that half of the experience stays broken until per-path WebSocket gating ships.
