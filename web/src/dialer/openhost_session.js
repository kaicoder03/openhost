...
export async function dialOhUrl(ohUrl, opts = {}) {
  ...
  // 2. Client identity. Callers may inject an explicit seed (e.g. the
  // web app derives it from a pairing code so sender + receiver share
  // a deterministic pubkey). Absent an override, fall back to the
  // IndexedDB-backed per-install seed.
  const clientSeed = opts.clientSeed
    ? (opts.clientSeed instanceof Uint8Array
        ? opts.clientSeed
        : new Uint8Array(opts.clientSeed))
    : await loadOrCreateClientSeed();
